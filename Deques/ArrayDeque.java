...
public class ArrayDeque<T> implements Deque<T> {
    ...
    public T removeLast() {
        ...
        back = decrement(back, data.length); // decrements back index position to account for the removal of item.
        T result = data[back];
        data[back] = null;
        size -= 1;
        if (needsDownsize()) {        //comment out
            resize(data.length / 2); //length is 3 at the time of the crash and will be 1 at once divided
        }
        return result;
    }
